<?php
	...
	if($db->conectar()){
		...
		$votos = "SELECT idPlaylist, cantidad_reproducciones FROM playlist";
		...
		while ($row = mysqli_fetch_assoc($resultadoVotos)){    
			...
			$datay[$i] = $row['cantidad_reproducciones'];   
			$i++;
		}  	
		...
		// Setup title
		$graph->title->Set('Cantidad de reproducciones por Playlist.');
		...
		$fileName = "graficoRankingVotosPlaylist.png";
