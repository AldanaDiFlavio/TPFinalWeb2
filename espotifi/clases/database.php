...
class database	{
				...
				public function buscar($nombre, $contrasena)
						{					
						$this->validar = mysqli_query ($this->conexion, "SELECT * from Usuarios WHERE nombre LIKE '$nombre' AND contrasena LIKE '$contrasena';")
				...
				public function habilita($nombreId)
						{
						$sql="SELECT Habilitado FROM usuarios WHERE Nombre = '$nombreId';";
						...
						if($ver["Habilitado"] == "true")
								{
								//deshabilita usuario
								$sql="UPDATE usuarios SET Habilitado = 'false' WHERE Nombre = '$nombreId';";
								$resultado = mysqli_query($this->conexion,$sql);
								}
								else{
									//habilita usuario
									$sql="UPDATE usuarios SET Habilitado = 'true' WHERE Nombre = '$nombreId';";
									$resultado = mysqli_query($this->conexion,$sql);
									}
						}	
				...
				public function listarUsuarios()
						{
						$query = mysqli_query($this->conexion, "SELECT * FROM usuarios;") or die ("Fallo la consulta");
						...
						while($ver = mysqli_fetch_assoc($query))
								...
								echo "<div style= 'text-align:center; float: left; border-style: solid; border-width: 0px; margin: 5px;' >";	
										...
										echo '<br><a href = "../php/exportarPDF.php"><button> Exportar </button></a>';
								echo  "</div>";
								}		
						}
